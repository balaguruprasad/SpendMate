...
function Button({
  ...
  VariantProps<typeof buttonVariants> & {
    asChild?: boolean
    /**
     * When true, shows a leading spinner and blocks interaction WITHOUT the
     * greyed `disabled` muting — keeps near-full opacity, sets `aria-busy`, and
     * applies `pointer-events-none` to guard against double-submit. The native
     * `disabled` attribute is only set when `disabled` is explicitly passed.
     */
    loading?: boolean
  }) {
  const Comp = asChild ? Slot.Root : "button"
  // When asChild, the Slot expects a single child — don't inject a spinner.
